...
bool c_projectile_sim::get_info( c_base_player* player, c_base_combat_weapon* weapon, const vector& angles, projectile_info_t& info ) {
	...
	case TF_WEAPON_ROCKETLAUNCHER: {
		if (weapon->m_item_definition_index() == Soldier_m_TheOriginal)
			//			(FIXED ISH)									this offset is still wrong, its still vertically offset on the original (afaik)
			tf2::get_projectile_fire_setup(player, angles, vector(0.0f, 0.0f, ducking ? 0.0 : -6.0f), position, angle, false);
		else
